	...
	return useQuery({
		...
		queryFn: async () => {
			const response = await axiosClient.get<User>(
				generateLink({
					route: [ROUTES.USER.BASE, ROUTES.USER.GET_ONE],
					params: { id },
				})
			);
	...
	return useMutation({
		mutationFn: async ({
			...
			const response = await axiosClient.patch<User>(
				generateLink({
					route: [ROUTES.USER.BASE, ROUTES.USER.UPDATE],
					params: { id },
				}),
				data
			);
		...
		onSuccess: (data) => {
			queryClient.invalidateQueries({
				queryKey: [USER_QUERY_KEY, data?.id],
			});
		},
	});
};
	...
	return useMutation({
		mutationFn: async (id: string) => {
			await axiosClient.delete(
				generateLink({
					route: [ROUTES.USER.BASE, ROUTES.USER.DELETE],
					params: { id },
				})
			);
		},
	...
	return useQuery({
		...
		queryFn: async () => {
			const response = await axiosClient.get(
				generateLink({
					route: [ROUTES.AUTH.BASE, ROUTES.AUTH.GET_DEVICES],
				})
			);
...
export const useDisconnectDevice = () => {
	...
	return useMutation({
		mutationFn: async (deviceId: string) => {
			...
			await axiosClient.delete(
				generateLink({
					route: [ROUTES.AUTH.BASE, ROUTES.AUTH.DISCONNECT_DEVICE],
					params: { id: deviceId },
				})
			);
		},
		...
		mutationFn: async () => {
			await axiosClient.delete(
				generateLink({
					route: [
						ROUTES.AUTH.BASE,
						ROUTES.AUTH.DISCONNECT_ALL_DEVICES,
					],
				})
			);
		},
		...
		mutationFn: async ({
			...
			const response = await axiosClient.post(
				generateLink({
					route: [
						ROUTES.USER.BASE,
						ROUTES.USER.CREATE_ENCRYPTION_KEY,
					],
				}),
				{
					salt,
		...
		mutationFn: async ({
			...
			const response = await axiosClient.patch(
				generateLink({
					route: [
						ROUTES.USER.BASE,
						ROUTES.USER.CHANGE_ENCRYPTION_KEY,
					],
				}),
				{
					salt,
...
export const useResetEncryptionKey = () => {
	...
	return useMutation({
		mutationFn: async ({
			...
			const response = await axiosClient.patch(
				generateLink({
					route: [ROUTES.USER.BASE, ROUTES.USER.RESET_ENCRYPTION_KEY],
				}),
				{
					salt,
					verifyToken,
...
export const useUpdateUserPassword = () => {
	return useMutation({
		mutationFn: async (password: string) => {
			...
			const response = await axiosClient.post(
				generateLink({
					route: [ROUTES.AUTH.BASE, ROUTES.USER.CHANGE_PASSWORD],
				}),
				{ password }
			);
			return response.data;
